...
def safe_api_request(
    ...
    try:
        ...
        logger.debug(
            f"API 응답 Content-Type: {response.headers.get('content-type', 'Unknown')}"
        )
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"JSON과 XML 파싱 모두 실패. JSON 에러: {json_error}, XML 에러: {xml_error}"
                )
    ...
    try:
        ...
        if "cmmMsgHeader" in data:
            ...
            error_msg = header.get(
                "returnAuthMsg", header.get("errMsg", "Unknown error")
            )
            ...
            if return_reason_code == "30":
                ...
                logger.error("💡 해결 방법:")
                logger.error(
                    "  1. 공공데이터포털(data.go.kr)에서 API 키를 다시 확인하세요"
                )
                logger.error(
                    "  2. API 키가 올바르게 .env 파일에 설정되었는지 확인하세요"
                )
                logger.error(
                    "  3. API 키에 특수문자가 포함되어 있으면 URL 인코딩 문제일 수 있습니다"
                )
                return False
            else:
                logger.error(
                    f"공휴일 API 에러 (코드: {return_reason_code}): {error_msg}"
                )
                return False
            ...
            if response_data.get("header", {}).get("resultCode") == "00":
                ...
                error_msg = response_data.get("header", {}).get(
