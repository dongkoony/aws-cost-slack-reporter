    ...
    try:
        ...
        try:
            response = client.files_upload_v2(
                channel=channel, file=tmp_file_path, filename=filename, title=title
            )
        finally:
            # 임시 파일 정리
            try:
...
def create_cost_report_blocks(
    ...
    blocks = [
        {
            "type": "header",
            "text": {"type": "plain_text", "text": "💰 AWS 비용 상세 리포트"},
        },
        {
            "type": "section",
            "text": {
...
def create_service_breakdown_blocks(
    service_costs: Dict[str, float], exchange_rate: float = 1300.0
) -> List[Dict[str, Any]]:
    """
    ...
    blocks = [
        {
            "type": "header",
            "text": {"type": "plain_text", "text": "💰 AWS 서비스별 상세 과금 현황"},
        },
        {
            "type": "section",
            "text": {
    ...
    if other_cost > 0:
        blocks.append(
            {
                "type": "section",
                "text": {"type": "mrkdwn", "text": f"*기타 서비스*\n${other_cost:.2f}"},
            }
        )
...
def send_cost_report(
    ...
    try:
        ...
        # 서비스별 내역 블록 생성 및 전송
        if service_costs:
            breakdown_blocks = create_service_breakdown_blocks(
                service_costs, exchange_rate
            )
        ...
        if chart_image:
            ...
            success = upload_file_to_slack(
                chart_image, filename, "AWS 월간 비용 분석 차트"
            )
            if not success:
