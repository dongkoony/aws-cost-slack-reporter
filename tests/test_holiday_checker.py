...
from src.holiday_checker import (check_holiday, get_date_range,
                                 get_monthly_date_range, is_business_day,
                                 should_send_report)
...
class TestHolidayChecker:
    ...
    @patch("src.holiday_checker.check_holiday")
    @patch("src.holiday_checker.is_business_day")
    def test_should_send_report_business_day(
        self, mock_is_business_day, mock_check_holiday
    ):
        """평일 리포트 전송 여부 테스트"""
        # 평일이면서 공휴일 아님
        mock_is_business_day.return_value = True
